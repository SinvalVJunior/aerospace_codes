...
class Aerospace
{
  public:
    ...
    //---------------------BME-------------------------
    //Funcoes de setup
    bool BME_begin(); //Uso basico

    //Funcoes de leitura de dados (as que utilizaremos em nosso arduino)
   ...
   //Structs associada à calibragem BME (Podem ser removidas)
    ...
    typedef struct
    {
        ...
        int8_t   dig_H6; ///< humidity compeOUTPUTnsation value
    } bme280_calib_data;
    ...
    //Funcao para uso avançado (consultar Datasheet)
...
//---------------------BME-------------------------
    ...
    uint8_t   _i2caddr;
    int32_t   _sensorID, t_fine;

    bool BME_init();

    void      BME_readCoefficients(void);
    bool      BME_isReadingCalibration(void); //Nao esta usando ainda
    ...
    //Structs avançadas (usadas na configuração do Sampling. Podem ser alteradas)
        // The config register
        struct config {
